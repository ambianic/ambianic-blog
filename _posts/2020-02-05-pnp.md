...
Its time to introduce a wonderful W3C standard called [WebRTC](https://webrtc.org/). Its a secure peer to peer communication protocol that all major browsers today support on dekstop and mobile. WebRTC solves a number of very hard problems, which is why it took 10 years of real world testing and adaptations for edge cases.

As of December 2019 the [W3C WebRTC 1.0 Recommendation](https://www.w3.org/TR/webrtc/) is official! Even better, its [implemented](https://wpt.fyi/results/webrtc?label=experimental&label=master&aligned) natively in all major modern browsers: Chrome, Safari, Firefox, Opera, Edge. No plugins required. While interop and polishing of the implementations will likely take more time, our testing shows that its already useful for the use case at hand. Open Source implementations of WebRTC are actively developed in [C, Java](https://opensource.google/projects/webrtc), [Go](https://github.com/pion/webrtc), and Python(https://github.com/aiortc/aiortc).

For the purpose of Ambianic UI and Ambianic Edge connectivity, we use the browser WebRTC stack and the Python aiortc implementations with an additional abstraction layer from [PeerJS](https://github.com/peers/peerjs/) browser and [PeerJS Python](https://github.com/ambianic/peerjs-python/stargazers).
