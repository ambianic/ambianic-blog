...
There is no free lunch. This approach is only as strong as its weakest link and if the basic assumptions don't hold then the user data is not safe:
* WiFi networks have been around more than 20 years. Yet they are still being [successfully attacked](https://cybersecurity.att.com/blogs/security-essentials/security-issues-of-wifi-how-it-works) by hackers exploiting poor security hygiene.
* Personal device security is another [common vector of attack](https://auth0.com/blog/ten-mobile-security-threats-and-what-you-can-do-to-fight-back/). If its compromised then access to the Edge device is also exposed. 

We will keep eyes peeled for community ideas and contributions to tighten up further user privacy protection. Some suggestions on the table include two factor authentication (2FA) for access to Ambianic Edge and biometric login for Ambianic UI (fingreprint or face recognition). What do you think is a good balance of usability vs safety? Let us know on [our twitter channel](https://twitter.com/ambianicai).
